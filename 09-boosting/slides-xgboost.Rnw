...
library with \pkg{R}/Python/Julia/Spark interface.
...
$J_2(\bmm) = \left\|c^{[m]}\right\|^2_2$ and $J_3(\bmm) = \left\|c^{[m]}\right\|_1$ are $L_2$ and $L_1$ regularizers of the terminal region scores $c_t^{[m]}, t=1,\dots,T^{[m]}$.

\lz

\textbf{Note:} In the following we define $J(\bmm) = \lambda J_1(\bmm) + \gamma J_2(\bmm) + \alpha J_3(\bmm)$ to simplify the notation.

\framebreak

Recall the way a tree base-learner $\bmm(x)$ can be fitted loss-optimally in gradient boosting:

$$
\tilde{c}^{[m]} = \argmin_{(c_1,\dots,c_{T^{[m]}})}\sum_{i = 1}^n L(\yi, \fmd(\xi) + \bmm(\xi, c_1,\dots,c_{T^{[m]}})).
$$

The direction of steepest descent for the update is then

$$
-\frac{\partial L(y, \fmd(x) + \bmm(x))}{\partial \bmm(x)}
$$
...
\textbf{Note:} $J(\bmm)$ is ommitted for now, but will be reintroduced later.

\framebreak

To approximate the loss in iteration $m$, use a second-order Taylor expansion around $\fmd(x)$:
\vskip  -1em
\begin{align*}
  &L(y, \fmd(x) + \bmm(x)) \approx \\
  &\qquad L(y, \fmd(x)) + g^{[m]}(x)\bmm(x) + \frac12 h^{[m]}(x)\bmm(x)^2,
\end{align*}

with gradient

$$
g^{[m]}(x) = \frac{\partial L(y, \fmd(x))}{\partial \fmd(x)}
$$

and Hessian

$$
h^{[m]}(x) = \frac{\partial^2 L(y, \fmd(x))}{\partial {\fmd(x)}^2}.
$$

\textbf{Note:} $g^{[m]}(x)$ are the negative pseudo-residuals $-r^{[m]}$ we use in standard gradient boosting to determine the direction of the update.

\framebreak

Since $L(y, \fmd(x))$ is a constant, the optimization criterion simplifies to
...
\intertext{
Defining $G^{[m]}_t$ and $H^{[m]}_t$ as the sum of the gradients and Hessians, respectively, in terminal node $t$, yields
}
...
\textbf{Note:} The $\frac12$ is added to the $L_2$ regularization to simplify the notation as shown in the second step.
This does not influence the estimation since we can just define $\lambda = 2\tilde\lambda$.
...
To evaluate the performance of a candidate split that divides the instances in region $R_t^{[m]}$ into a left and right node we use the \emph{risk reduction} achieved by that split:
$$
\tilde S_{LR} =
...
XGBoost also allows sparsity-aware split finding, which means that for a large number of zero (or missing) values, a default direction in each node is introduced.
...
Furthermore it uses \emph{feature subsampling} similar to the random forest. Only a random subset of all features is considered for each split.
...
After an update the joint contribution of the dropped tree and the new tree to the ensemble are scaled to avoid overshooting.

\end{vbframe}

\begin{vbframe}{Storage and out-of-memory calculation}

The data is stored in \emph{blocks.}
...
XGBoost is an extremely powerful method, but also makes it hard to configure correctly.
Overall eight hyperparameters have to be set, which is difficult to do in practice and almost always requires tuning.

\lz

Different split finding algorithms can be selected, which allows XGBoost to be efficient even on very large data sets.
